...
- Compound scaled units should use named scaled units when available (e.g.,
  `kilowatt.scale * hour.scale` not `watt.scaled(KILO).scale * hour.scale`). Use
  `roundScale()` from `dim-unit` when the composed expression has IEEE 754 noise
  (e.g., `centimeter.scale ** 3`). Do not use it on values that are already the
  best float64 approximation (e.g., `1000 / 3600`).
