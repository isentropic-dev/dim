...
type D<T extends Partial<Record<"L" | "M" | "T" | "I" | "Θ" | "N" | "J", Exp>>> = WithDefaults<typeof isq.dims, T>;
...
export const acceleration: QuantityFactory<Acceleration> = isq.factory({ L: 1, T: -2 });
...
export const energy: QuantityFactory<Energy> = isq.factory({ L: 2, M: 1, T: -2 });
...
export const pressure: QuantityFactory<Pressure> = isq.factory({ L: -1, M: 1, T: -2 });
...
export const voltage: QuantityFactory<Voltage> = isq.factory({ L: 2, M: 1, T: -3, I: -1 });

/** Resistance (L²·M·T⁻³·I⁻²) */
export type Resistance = D<{ L: 2, M: 1, T: -3, I: -2 }>;
export const resistance: QuantityFactory<Resistance> = isq.factory({ L: 2, M: 1, T: -3, I: -2 });

/** Capacitance (L⁻²·M⁻¹·T⁴·I²) */
export type Capacitance = D<{ L: -2, M: -1, T: 4, I: 2 }>;
export const capacitance: QuantityFactory<Capacitance> = isq.factory({ L: -2, M: -1, T: 4, I: 2 });

/** Inductance (L²·M·T⁻²·I⁻²) */
export type Inductance = D<{ L: 2, M: 1, T: -2, I: -2 }>;
export const inductance: QuantityFactory<Inductance> = isq.factory({ L: 2, M: 1, T: -2, I: -2 });

/** Conductance (L⁻²·M⁻¹·T³·I²) */
export type Conductance = D<{ L: -2, M: -1, T: 3, I: 2 }>;
export const conductance: QuantityFactory<Conductance> = isq.factory({ L: -2, M: -1, T: 3, I: 2 });

/** MagneticFlux (L²·M·T⁻²·I⁻¹) */
export type MagneticFlux = D<{ L: 2, M: 1, T: -2, I: -1 }>;
export const magneticFlux: QuantityFactory<MagneticFlux> = isq.factory({ L: 2, M: 1, T: -2, I: -1 });

/** MagneticFluxDensity (M·T⁻²·I⁻¹) */
export type MagneticFluxDensity = D<{ M: 1, T: -2, I: -1 }>;
export const magneticFluxDensity: QuantityFactory<MagneticFluxDensity> = isq.factory({ M: 1, T: -2, I: -1 });

/** LuminousFlux (J) */
export type LuminousFlux = D<{ J: 1 }>;
export const luminousFlux: QuantityFactory<LuminousFlux> = isq.factory({ J: 1 });

/** Illuminance (L⁻²·J) */
export type Illuminance = D<{ L: -2, J: 1 }>;
export const illuminance: QuantityFactory<Illuminance> = isq.factory({ L: -2, J: 1 });

/** AbsorbedDose (L²·T⁻²) */
export type AbsorbedDose = D<{ L: 2, T: -2 }>;
export const absorbedDose: QuantityFactory<AbsorbedDose> = isq.factory({ L: 2, T: -2 });

/** CatalyticActivity (T⁻¹·N) */
export type CatalyticActivity = D<{ T: -1, N: 1 }>;
export const catalyticActivity: QuantityFactory<CatalyticActivity> = isq.factory({ T: -1, N: 1 });
